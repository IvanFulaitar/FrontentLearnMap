...
const NODE_INFO: Record<NodeId, { tag: string; text: string; note: string }> = {
  body: { tag: "body", text: "(головний, батьківський елемент)", note: "body — батьківський елемент для h1, p і button: усі вони знаходяться всередині нього, тому в дереві вони показані як його \"діти\"." },
  h1: { tag: "h1", text: "Кав'ярня «Аромат»", note: "Текст усередині h1 — саме його змінює команда, яку показано в прикладі нижче (без зміни самого файлу на диску)." },
  p: { tag: "p", text: "Смачна кава з 8:00", note: "Звичайний абзац — окремий елемент, який знаходиться на тому самому рівні дерева, що й h1 і button (сусідній елемент, не батько й не дитина для них)." },
  button: { tag: "button", text: "Купити", note: "Саме до таких елементів у реальному коді \"прив'язують\" реакцію на клік користувача." },
};
...
// змінюється лише DOM у пам'яті браузера`}
      ...
      <DemoKeyTakeaway>
        DOM — це не сам HTML-файл, а жива структура елементів, яку браузер тримає у своїй пам'яті. Клік по кнопці
        вище показує те саме, що робить рядок коду document.getElementById(...).textContent у реальному JavaScript:
        змінюється те, що бачить користувач на екрані, а сам файл на диску лишається незмінним.
      </DemoKeyTakeaway>
    </div>
  );
}
