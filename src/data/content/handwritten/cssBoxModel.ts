...
export const cssBoxModelOverrides: Record<string, LessonOverride> = {
  "Box-модель на практиці": {
    ...
    whatIsIt: "У попередньому модулі ми розібрались із селекторами, каскадом і CSS-змінними — тобто як писати й підключати правила. Тепер саме час розібратись, що ці правила роблять з розміром елементів на сторінці, — і почати з найважливішого: box-моделі.\n\nBox-модель — правило, за яким браузер рахує розмір кожного HTML-елемента: content (вміст) → padding (внутрішній відступ) → border (рамка) → margin (зовнішній відступ), шар за шаром зсередини назовні. box-sizing вирішує, чи padding/border входять у заданий width, чи додаються поверх нього.\n\ncontent це сам текст/картинка/вміст кнопки, padding це простір між вмістом і рамкою, border це сама рамка (товщина + стиль + колір), margin це простір зовні елемента до сусідів — дивись схему нижче.",
    visualExplanation: {
...
body {
  ...
  outline-offset: 3px;
    ...
    dontDoThis: { code: `.card {\n  width: 300px;\n  padding: 24px;\n  /* без box-sizing: border-box */\n}`, explanation: "На перший погляд усе гаразд — код читається однозначно: картка шириною 300px з відступом 24px усередині. Але в браузері вона займе 348px (300 + 24 зліва + 24 справа), і якщо поруч стоїть ще одна така картка чи фіксований контейнер, розмітка \"розповзається\" або з'являється неочікуваний перенос/скрол. Причина в тому, що за замовчуванням діє content-box: width задає ширину лише самого вмісту, а padding і border додаються зверху, поза заданим числом. Виправлення: додати box-sizing: border-box (найкраще — глобально, одним правилом на весь проєкт), і тоді padding з border почнуть рахуватись усередині заданих 300px. Перевірити можна в DevTools — вкладка Computed чи Box Model покаже реальну ширину елемента, а не ту, що написана в CSS." },
...
:root {
  ...
  "Margin проти padding": {
    ...
    whatIsIt: "У попередньому уроці ми познайомились із чотирма шарами box-моделі — content, padding, border, margin — і навчились рахувати реальний розмір елемента через box-sizing: border-box. Тепер розберемось детальніше з двома шарами, які новачки плутають найчастіше: padding і margin.\n\npadding — простір усередині елемента, між його рамкою і вмістом; він частина \"клікабельної\"/видимої області фону. margin — простір зовні, між елементом і його сусідами; фон елемента туди не поширюється.\n\nОбидва мають короткий (shorthand) запис: одне значення — усі 4 сторони, два значення — верх/низ і ліво/право, три — верх, ліво/право, низ, чотири — за годинниковою стрілкою (верх → право → низ → ліво). Сучасні логічні властивості padding-inline/padding-block і margin-inline/margin-block замінюють пари left/right і top/bottom — тут inline/block не мають нічого спільного з display: inline/block з попереднього уроку: inline означає \"вздовж напрямку тексту\" (для української це горизонталь: ліво+право одразу), block означає \"впоперек напрямку тексту\" (вертикаль: верх+низ одразу).",
...
.title { margin-bottom: var(--space-md); }`,
    ...
    dontDoThis: { code: `.button {\n  margin: 12px 24px; /* мало бути padding */\n}`, explanation: "Результат на перший погляд не викликає підозр — навколо кнопки з'явився якийсь відступ, і візуально стало ніби просторіше. Але текст усередині кнопки як був притиснутий до країв, так і лишився — фон кнопки не виріс, а просто відсунувся від сусідніх елементів. Причина в тому, що margin додає простір ЗА межами елемента (до сусідів), а не між текстом і рамкою самого елемента — для цього призначений padding. Виправлення: замінити на padding: 12px 24px, щоб текст отримав \"дихання\" всередині кнопки, а за потреби окремо додати невеликий margin для відстані від сусідів. Перевірити можна в DevTools — вкладка Box Model одразу покаже, який шар (padding чи margin) реально збільшений." },
...
.second { visibility: hidden; } /* місце другого елемента залишиться порожнім */`,
    ...
    dontDoThis: { code: `span { width: 200px; height: 50px; background: coral; }`, explanation: "Код виглядає цілком робочим — задані width, height і колір фону, синтаксичних помилок немає. Але в браузері span лишається тонкою смужкою по висоті тексту: жодного 200×50, жодного coral-прямокутника заданого розміру. Причина в тому, що span — inline-елемент за замовчуванням, а inline-елементи за задумом течуть у тексті й повністю ігнорують width і height, реагуючи лише на розмір свого вмісту. Виправлення: додати display: inline-block (якщо потрібно, щоб елемент і далі тік у рядку тексту) або display: block (якщо потрібен окремий рядок) — тоді width/height почнуть діяти. Перевірити можна в DevTools: вкладка Computed для такого span покаже реальні (ігноровані) значення width/height, поки display лишається inline." },
