...
export const cssBoxModelOverrides: Record<string, LessonOverride> = {
  "Box-модель на практиці": {
    ...
    whatIsIt: "Box-модель — правило, за яким браузер рахує розмір кожного HTML-елемента: content (вміст) → padding (внутрішній відступ) → border (рамка) → margin (зовнішній відступ), шар за шаром зсередини назовні. box-sizing вирішує, чи padding/border входять у заданий width, чи додаються поверх нього. Схема: [Margin [ Border [ Padding [ Content ] ] ] ] — content це сам текст/картинка/вміст кнопки, padding це простір між вмістом і рамкою, border це сама рамка (товщина + стиль + колір), margin це простір зовні елемента до сусідів.",
    whyUseIt: "Без розуміння box-моделі незрозуміло, чому блок став ширшим за очікуване, чому кнопка виглядає затісною, чому картки з'їхали, чому з'явився горизонтальний скрол, чому padding збільшує елемент, а margin — ні, і чому фон не поширюється на margin. Box-модель використовується буквально всюди: кнопки, картки товарів, форми, секції, header/footer, модальні вікна, таблиці, layout, sidebar, навігація, hero-секції, grid і flexbox. Наприклад дизайнер у Figma показує картку 320px завширшки з padding 24px, рамкою 1px і радіусом 16px — без border-box ця картка в браузері вийде фактично на 50px ширшою, ніж задумано.",
...
:root {
  ...
  "Margin проти padding": {
    ...
    whatIsIt: "padding — простір усередині елемента, між його рамкою і вмістом; він частина \"клікабельної\"/видимої області фону. margin — простір зовні, між елементом і його сусідами; фон елемента туди не поширюється. Обидва мають короткий (shorthand) запис: одне значення — усі 4 сторони, два значення — верх/низ і ліво/право, три — верх, ліво/право, низ, чотири — за годинниковою стрілкою (верх → право → низ → ліво). Сучасні логічні властивості padding-inline/padding-block і margin-inline/margin-block замінюють пари left/right і top/bottom.",
    whyUseIt: "Плутанина margin/padding — топ-1 помилка новачків: результат виглядає однаково на око, але поводиться по-різному (фон, клікабельність, схлопування). Відступи є буквально всюди в реальному проєкті: між секціями, між картками, всередині кнопок і форм, між заголовком і текстом, між пунктами меню, навколо зображень, у footer. Неправильний вибір робить сайт неакуратним: кнопка занадто тісна, текст прилипає до рамки, картки стоять одна на одній, або навпаки — секції не мають \"повітря\".",
...
.title { margin-bottom: var(--space-md); }`,
  ...
  "Display: block, inline, none": {
    ...
    whatIsIt: "display визначає базову поведінку елемента: block займає весь рядок, починається з нового рядка і поважає width/height/margin/padding; inline тече в рядку тексту, займає лише ширину свого вмісту й ігнорує width/height; inline-block — проміжний варіант: тече в рядку, як inline, але поважає width/height, як block; none повністю прибирає елемент з розмітки (місце не резервується, і зазвичай елемент недоступний для скрінрідера). За замовчуванням block — це div, p, h1-h6, section, article, header, footer, main, nav, ul, ol, li, form; inline — це span, a, strong, em, label, small, code.",
    whyUseIt: "\"Чому мій width не працює?\" — у більшості випадків елемент inline (наприклад, <span>), а inline-елементи ігнорують width/height за задумом. display — фундамент, без якого важко зрозуміти CSS layout, Flexbox, Grid і адаптивну верстку: від нього залежить, чи заголовок починається з нового рядка, чи посилання можна клацнути по всій області, як показати чи сховати мобільне меню, чому клікабельна зона пункту меню занадто маленька.",
