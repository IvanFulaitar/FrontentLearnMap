...
// Порядок: 1, 2, 3 — НЕ 1, 3, 2, попри delay: 0`,
        lineNotes: ["Навіть з delay: 0 callback потрапляє в черзі ПІСЛЯ всього поточного синхронного коду — цикл подій (event loop) обробляє відкладені callback-и лише коли стек викликів (call stack, список функцій, які JavaScript зараз виконує одна за одною) стає порожнім.", "Це доводить: setTimeout НІКОЛИ не виконується синхронно \"негайно\", навіть із нульовою затримкою."],
      },
    ],
...
`,
  ...
  "Основи скасування запитів": {
    ...
    whenToUse: ["Живий пошук/автозаповнення — кожен новий символ повинен скасовувати ПОПЕРЕДНІЙ запит, ще не завершений.", "Скасування fetch-запиту, коли частина інтерфейсу, що чекала на відповідь, видаляється зі сторінки раніше, ніж запит завершився.", "Надання користувачу явної кнопки \"Скасувати\" для довготривалої операції."],
...
// renderResults покаже застарілі результати для "a" останніми`,
      ...
      {
        before: "Загальний прийом: функція, що запускає запит, повертає СВОЮ ВЛАСНУ функцію \"прибирання\" (cleanup) — виклик коду сам вирішує, коли її запустити, наприклад, коли частина інтерфейсу закривається до завершення запиту:",
        code: `function loadUserPanel(userId) {
  ...
  // Повертаємо функцію "прибирання" замість того, щоб самим вирішувати, коли скасовувати
  return function cleanup() {
    controller.abort();
  };
}

const stopLoading = loadUserPanel(42);

// Якщо панель користувача закривається раніше, ніж прийшла відповідь:
closeButton.addEventListener("click", () => {
  stopLoading(); // скасовує запит, якщо він ще не завершився
  userPanel.remove();
});`,
        lineNotes: ["loadUserPanel не вирішує САМА, коли скасовувати запит — вона лише повертає готову функцію cleanup, а МОМЕНТ її виклику визначає той код, що використовує loadUserPanel (тут — обробник кліку на кнопці закриття).", "Цей прийом — \"запусти роботу, поверни спосіб її скасувати\" — дуже поширений патерн. Пізніше, у курсі React, ти побачиш майже ідентичну конструкцію: функція useEffect() теж може повертати функцію cleanup, яку React викликає автоматично, коли компонент зникає зі сторінки чи оновлюється — саме тому вже зараз корисно розуміти цей прийом на простому DOM-прикладі."],
      },
    ],
    ...
    bestPractices: ["Створюй новий AbortController для КОЖНОГО нового запиту, що потенційно потребує скасування.", "Скасовуй ПОПЕРЕДНІЙ запит перед стартом нового у сценаріях типу живого пошуку.", "Завжди перевіряй error.name === \"AbortError\" окремо від реальних помилок мережі.", "Якщо запит запускається разом із появою якоїсь частини інтерфейсу, повертай функцію cleanup, яка його скасовує — і викликай цю функцію, коли та частина інтерфейсу зникає (закривається, видаляється, замінюється новою)."],
