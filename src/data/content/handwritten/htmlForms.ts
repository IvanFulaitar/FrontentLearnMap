...
export const htmlFormsOverrides: Record<string, LessonOverride> = {
  "Структура форми та підписи": {
    whatIsIt: "<form> обгортає поля вводу і визначає, куди (action) і як (method) відправити дані. Кожне поле має <label>, зв'язаний через for/id, та name — саме name іде на сервер. Без label скрінрідер не знає, що означає поле; без name поле взагалі не відправиться.",
  ...
  "Типи полів і атрибути": {
    whatIsIt: "type у <input> визначає клавіатуру на мобільному й вбудовану валідацію: email перевіряє формат, number приймає лише цифри, tel показує цифрову клавіатуру. HTML підтримує понад 20 типів; readonly/disabled/pattern/minlength/maxlength додатково керують поведінкою поля.",
...
<input type="number" id="guests" name="guests" min="1" max="12" required />`,
  ...
  "Textarea, select і fieldset": {
    whatIsIt: "<textarea> — багаторядковий текст. <select>+<option> — випадаючий список (значення для сервера задає value; групувати варіанти можна через <optgroup>). <fieldset>+<legend> — групування пов'язаних полів з підписом групи.",
...
<select id="city" name="city">
  ...
  "Чекбокси, радіокнопки і валідація": {
    whatIsIt: "checkbox — так/ні або кілька незалежних варіантів. radio (з однаковим name) — рівно один варіант із кількох. required/pattern/min/max/minlength/maxlength — вбудована HTML-валідація без JS; novalidate вимикає її повністю.",
...
<label><input type="checkbox" name="agree" required /> Погоджуюсь з умовами</label>`,
  ...
  "Доступні повідомлення про помилки": {
    whatIsIt: "aria-describedby зв'язує поле з текстом помилки під ним. aria-invalid=\"true\" позначає поле як некоректне. role=\"alert\" (або aria-live) змушує скрінрідер одразу озвучити повідомлення, щойно воно з'явилось на сторінці.",
