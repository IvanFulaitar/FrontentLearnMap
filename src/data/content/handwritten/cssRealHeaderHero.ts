...
export const cssRealHeaderHeroOverrides: Record<string, LessonOverride> = {
  "Header і sticky-навігація": {
    whatIsIt: "У попередніх модулях ми навчились розташовувати блоки через Flexbox і Grid. Тепер починаємо збирати справжні компоненти сайту кав'ярні — і перший з них, header, потребує ще однієї властивості, яку ми досі не використовували: position.\n\nposition: sticky тримає header видимим угорі екрана під час прокрутки, доки не досягне заданого краю (top: 0). На відміну від fixed, sticky-елемент поводиться як звичайний, доки не \"прилипне\".",
    ...
    whenNotToUse: ["Не використовуй position: fixed для header, якщо не потрібно, щоб він \"вилітав\" з нормального потоку макета, — sticky зазвичай зручніший.", "Не забувай z-index — без нього sticky header може випадково опинитись позаду іншого елемента сторінки, у якого z-index заданий вищим.", "Не роби background надто прозорим разом з blur — текст навігації стає погано читабельним на строкатому фоні під ним."],
...
html {
  scroll-padding-top: 80px;
    ...
    dontDoThis: { code: `.site-header {\n  position: sticky;\n  /* без top */\n}`, explanation: "У коді все виглядає правильно: position: sticky написано, помилок у консолі немає. Але під час прокрутки сторінки header просто зникає нагорі разом з рештою контенту, наче sticky взагалі не задано. Причина в тому, що position: sticky сам по собі лише вмикає РЕЖИМ \"прилипання\" — він нічого не каже браузеру, ДЕ саме елемент повинен прилипнути; цю точку задає окрема властивість top (або left/right/bottom), і без неї браузер не має орієнтира й лишає елемент у звичайному потоці. Виправлення: додати top: 0 — тепер браузер точно знає, що header має \"зупинитись\", коли досягне верхнього краю екрана. Перевірити можна в інструментах розробника: прибери top з правила — header одразу перестане прилипати під час прокрутки, поверни top: 0 — прилипання відновиться." },
  ...
  "Мобільне меню без JavaScript": {
    whatIsIt: "У попередньому уроці ми зробили header липким угорі екрана. Але на вузькому екрані повний ряд посилань навігації просто не влазить у header — потрібен спосіб приховати їх за кнопкою-гамбургером і показувати лише за запитом.\n\nПатерн \"checkbox hack\": прихований <input type=\"checkbox\"> + <label> як кнопка-гамбургер + CSS-селектор :checked показує/ховає меню — усе на чистому CSS, без жодного JS.",
    ...
    comparisonTable: {
      headers: ["Підхід", "Потребує JavaScript?", "Коли доречний"],
      rows: [
        ["Checkbox hack (input + label + :checked)", "Ні", "Потрібна гнучка стилізація довільного контенту меню чистим CSS"],
        ["<details>/<summary>", "Ні", "Найпростіше меню чи акордеон без кастомної анімації"],
        ["useState (React, пізніше в курсі)", "Так", "Коли проєкт уже на React і станом керує сам компонент"],
      ],
    },
    codeWalkthroughs: [
      {
...
.menu-toggle-input:checked ~ .mobile-menu {
  display: block;
    ...
    dontDoThis: { code: `.menu-toggle-input { display: none; }`, explanation: "Візуально все гаразд: чекбокс не видно, іконка-гамбургер працює мишкою. Але користувач, що переходить по сайту клавішею Tab (без миші, наприклад через порушення зору чи моторики), взагалі не може дістатись до перемикача меню — фокус \"перескакує\" через нього, наче його немає на сторінці. Причина в тому, що display: none повністю прибирає елемент не лише з екрана, а й з порядку табуляції та з дерева доступності — для браузера це так, ніби елемента взагалі не існує, хоча функціонально він критично потрібен (це і є сам перемикач меню). Виправлення: замінити display: none на клас .visually-hidden (position: absolute, розмір 1×1px, overflow: hidden) — елемент лишається невидимим візуально, але доступним з клавіатури й для скрінрідерів. Перевірити можна, натискаючи Tab від початку сторінки: з .visually-hidden фокус дійде до перемикача меню, з display: none — проскочить повз нього." },
...
.visually-hidden {
  ...
  "Hero-секція": {
    whatIsIt: "У попередньому уроці ми сховали навігацію за гамбургер-кнопкою на вузьких екранах. Тепер переходимо до наступного великого блоку сторінки — того, що відвідувач бачить одразу після header, ще до прокрутки.\n\nHero — перший великий екран сайту: великий заголовок, короткий опис, одна головна дія. Стилізується як flex-колонка з вертикальним і горизонтальним центруванням, min-height замість height.",
...
@media (max-width: 768px) {
  .hero {
    ...
    dontDoThis: { code: `.hero {\n  height: 100vh;\n  overflow: hidden;\n}`, explanation: "На десктопі, де текст короткий, hero виглядає ідеально: рівно на весь екран, нічого не обрізано. Але щойно текст стає довшим (наприклад, переклад заголовка іншою мовою виявився довшим за оригінал, або на вузькому мобільному екрані той самий текст займає більше рядків), нижня частина hero — опис чи навіть кнопка — просто зникає з очей, хоча в коді вона є. Причина в тому, що height: 100vh жорстко фіксує висоту блоку рівно на висоту екрана, а overflow: hidden обрізає все, що в цю висоту не влізло, замість того щоб дати блоку вирости під реальний обсяг контенту. Виправлення: замінити height: 100vh на min-height: 70vh (без overflow: hidden) — блок і надалі займає значну частину екрана, але росте, якщо контенту раптом стало більше. Перевірити можна, штучно подовживши текст заголовка чи опису в DevTools: з min-height блок плавно зросте, з height: 100vh + overflow: hidden — частина тексту зникне без сліду." },
  ...
  "Фонові зображення і object-fit": {
    ...
    whatIsIt: "У попередньому уроці ми зверстали hero з суцільним кольором чи двома колонками. Останній штрих, що робить hero атмосферним, а не просто текстовим блоком, — фонове фото.\n\nbackground-image + background-size: cover — зображення як фон блоку, обрізане під його розмір. Для <img>-тегів (не фонів) та сама ідея реалізується через object-fit: cover.",
...
.menu-card-image {
  ...
  object-position: top;
    ...
    dontDoThis: { code: `.hero {\n  background-image: url("interior.jpg");\n  /* без background-size */\n}`, explanation: "У коді немає жодної помилки: шлях до фото правильний, властивість написана без друкарських помилок. Але на екрані замість одного великого атмосферного фото видно дрібну повторювану плитку з того самого зображення, розкидану по всьому блоку, — виглядає як зламаний, невдалий фон. Причина в тому, що background-image сам по собі нічого не каже браузеру про МАСШТАБ зображення відносно блоку; за замовчуванням background-size дорівнює auto, тобто фото показується в оригінальному пікселному розмірі файлу, і якщо цей розмір менший за блок, background-repeat (теж за замовчуванням repeat) заповнює решту простору повторами. Виправлення: додати background-size: cover — тепер зображення масштабується так, щоб повністю заповнити блок без повторів, зберігаючи пропорції. Перевірити можна в DevTools: прибери background-size — фон одразу розпадеться на плитку, поверни cover — стане одним цільним зображенням." },
